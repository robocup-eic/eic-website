...
export default function Typography({ type, className, children, custom }: TypographyProps) {
  ...
  switch (type) {
    ...
    case "h2":
      return (
        <h2 className={custom && className ? getClassName(className) : getClassName("text-2xl font-bold")}>
          {children}
        </h2>
      )
    case "h3":
      return (
        <h3 className={custom && className ? getClassName(className) : getClassName("text-2xl font-medium")}>
          {children}
        </h3>
      )
    case "p":
      return (
        <p className={custom && className ? getClassName(className) : getClassName("text-lg font-medium")}>
          {children}
        </p>
      )
    case "caption":
